...
from CarlaEnv.carla_route_env_spawn import CarlaRouteEnvSpawn as CarlaEnvSpawn
...
def spawn_rl_agent():
    ...
    print("Running eval...")
    spawn_count = 2
    run_eval(env, model, spawn_count, video_filename=args.record_to_file)
...
def run_eval(env, model, spawncount, video_filename=None):
    # Init test env
...
if __name__ == "__main__":
    ...
    #     threads.append(threading.Thread(target=spawn_rl_agent))
    # for threadnum in range(len(threads)):
    ...
    # processes = []
    # for i in range(spawn_count):
    #     processes.append(multiprocessing.Process(target=spawn_rl_agent))
    # for pnum in range(len(processes)):
